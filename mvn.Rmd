...
saveRDS(res, file = "./mvn-post-inf-5.Rdata")
```

## Analysis
```{r}
res <- readRDS("./mvn-post-inf-5-rint.Rdata")
assignments <- get_cluster_assignments(data=df, x=data_mat, res=res, K = 5, niter=500)
```

```{r}
# idk why the axis labels aren't showing up
stacked_barplot(assignments, orderby="assignment", K = 5, binary = F, do_save = F)
#stacked_barplot(assignments, orderby="position", K = 3, binary = F, do_save = T)
```
...
plot_list[[10]]
#ggsave("./plots/normal-triangle-k=3.png", width = 6, height = 4)
```
